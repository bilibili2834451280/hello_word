 ...
 */
@WebServlet("/ShowExaminitallans")
public class ShowExaminitallans extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public ShowExaminitallans() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < tst_name_no.size(); i+=2) {
			ans += "<dd><a href='javascript:refresh2(\"" + tst_name_no.get(i+1) + "\");'><%=" + tst_name_no.get(i) + "%></a></dd>";
		}
		
		if(ans.length()<=0) {
			for (int i = 0; i < 3; i++)
				ans += "<dd><a href='javascript:refresh2(\"" + i + "\");'>testtestµÄ´ð°¸</a></dd>";
		}
